...
func builtins_wrapper{pedersen_ptr: HashBuiltin*}(num_a: felt, num_b: felt) {
    let (pedersen_ptr, result: felt) = get_hash(pedersen_ptr, num_a, num_b);

    return ();
}

func builtins_wrapper_iter{pedersen_ptr: HashBuiltin*}(
...
func main{pedersen_ptr: HashBuiltin*}() {
    let num_a = 123568;
    let num_b = 5673940;
    builtins_wrapper_iter(num_a, num_b, 5000);
